...
int CComboUI::SelectString(LPCTSTR lpszString, int nStartAfter/* = 0*/)
{
	for (int i = nStartAfter; i < GetCount(); i++)
	{
		CControlUI* pListItem = GetItemAt(i);
		CStdString strListItem;
		if (NULL != pListItem)
		{
			strListItem = pListItem->GetText();
		}
		if (0 == _tcscmp(lpszString, strListItem))
		{
			SelectItem(i);
			return i;
		}
