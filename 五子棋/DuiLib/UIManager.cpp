...
bool CPaintManagerUI::AttachDialog(CControlUI* pControl)
{
	...
	// 删除已有的控制树;
	// 我们可能会在这个函数中得到一个事件或者类似的结果，所以我们不能只删除该对象，而应将调用代码的内部内存释放;
	// 我们会延误清理工作;
	if( m_pRoot != NULL )
	{
...
bool CPaintManagerUI::InitControls(CControlUI* pControl, CControlUI* pParent /*= NULL*/)
{
	...
	pControl->SetManager(this, pParent, true);
...
{
	// 如果我们在布局构建的过程中，我们可以延长控件焦点的设置直到下一个重绘工作;
...
{
	int iPos = m_aNotifiers.Find(pNotifier);
	if (iPos < 0)
	{
...
{
	int iPos = m_aPreMessageFilters.Find(pFilter);
	if (iPos < 0)
	{
...
{
	int iPos = m_aMessageFilters.Find(pFilter);
	if (iPos < 0)
	{
...
{
	int iPos = m_aPostPaintControls.Find(pControl);
	if (iPos < 0)
	{
