...
void CLabelUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	{
		m_uTextStyle &= ~(DT_LEFT | DT_CENTER | DT_RIGHT | DT_TOP | DT_VCENTER | DT_BOTTOM);
		if( 0 == _tcscmp(pstrValue, _T("left")) )
		{
			m_uTextStyle |= DT_LEFT;
		}
		else if( 0 == _tcscmp(pstrValue, _T("center")) )
		{
			m_uTextStyle |= DT_CENTER;
		}
		else if( 0 == _tcscmp(pstrValue, _T("right")) )
		{
			m_uTextStyle |= DT_RIGHT;
		}
		else if( 0 == _tcscmp(pstrValue, _T("top")) )
		{
			m_uTextStyle |= DT_TOP;
		}
		else if( 0 == _tcscmp(pstrValue, _T("vcenter")) )
		{
			m_uTextStyle |= DT_VCENTER;
		}
		else if( 0 == _tcscmp(pstrValue, _T("bottom")) )
		{
...
void CProgressUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    ...
    else if( _tcscmp(pstrName, _T("tstrValue")) == 0 ) SetValue(_ttoi(pstrValue));
    else CLabelUI::SetAttribute(pstrName, pstrValue);
}
...
void CSliderUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_BUTTONUP )
    {
        ...
        m_pManager->SendNotify(this, _T("tstrValuechanged"));
        Invalidate();
        return;
    }
    if( event.Type == UIEVENT_CONTEXTMENU )
...
void CScrollBarUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    ...
    else if( _tcscmp(pstrName, _T("tstrValue")) == 0 ) SetScrollPos(_ttoi(pstrValue));
