...
bool CStdPtrArray::Remove(int iIndex)
{
	...
	{	
		// 内存移动拷贝，再给iIndex赋值;
		memmove(&m_ppVoid[iIndex], &m_ppVoid[iIndex+1], (m_nCount - iIndex) * sizeof(LPVOID));
...
bool CStdValArray::Remove(int iIndex)
{
	...
	{
		// 内存移动拷贝，再给iIndex赋值;
		memmove(&m_pVoid[iIndex], &m_pVoid[iIndex+1], (m_nCount - iIndex) * m_iElementSize);
